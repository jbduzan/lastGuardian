...
func tween_scale() -> void:
	...
	var initialScaleValue = sprite.scale
	var newScaleValue = initialScaleValue * scale_amount
	
	# Next we scale the sprite from its current scale to the scale amount (in 1/10th of the scale duration)
	tween.tween_property(sprite, "scale", newScaleValue, scale_duration * 0.1).from_current()
	...
	tween.tween_property(sprite, "scale", initialScaleValue, scale_duration * 0.9).from(newScaleValue)
